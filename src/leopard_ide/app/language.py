...
from leopard_ide.app.themes import SyntaxColors
...
class LanguageProvider(ABC):
    ...
    def compile(self, source: str, name: str, output_dir: Path, terminal: "TerminalPane") -> None:
        """Compile `source` into a standalone executable named `name` under
        `output_dir`, writing progress/errors to `terminal`. Optional — the
        default reports that this language doesn't support compiling."""
        terminal.write(f"{self.name} does not support compiling to a standalone executable.\n")
