...
from leopard_ide.app.doc_panel import DocPanel
from leopard_ide.app.editor import CodeEditor
from leopard_ide.app.file_browser import FileBrowserWidget
from leopard_ide.app.find_replace import FindReplaceDialog
from leopard_ide.app.language import LanguageProvider
from leopard_ide.app.leopard_language import LeopardLanguageProvider
from leopard_ide.app.settings import SettingsManager
from leopard_ide.app.settings_dialog import SettingsDialog
from leopard_ide.app.terminal import TerminalPane
from leopard_ide.app.themes import ThemeManager


class MainWindow(QMainWindow):
    """Generic IDE shell with menu, toolbar, file browser, editors, and console."""

    def __init__(self, parent=None, docs_dir: Optional[Path] = None):
        ...
        self.docs_dir = docs_dir or (
            Path(__file__).resolve().parent.parent.parent.parent / "user-docs"
        )
        self._console_position = "bottom"
        self._setup_ui()
        self._setup_menus()
        self._setup_toolbar()
    ...
    def _setup_ui(self) -> None:
        ...
        self.doc_panel = DocPanel(self.docs_dir, self)
        self.doc_panel.setMinimumWidth(280)
        self.doc_panel.closed.connect(lambda: self.toggle_docs_action.setChecked(False))
        self.doc_panel.hide()
    ...
    def _rebuild_content_layout(self) -> None:
        """(Re)build the editor/console/docs splitter tree for the current
        console position. Docs sit to the right of the console when the
        console is on the right, or to the right of the editor (above the
        console) when the console is below the editor."""
        old_content_splitter = self.content_splitter

        if self._console_position == "right":
            new_content = QSplitter(Qt.Orientation.Horizontal)
            new_content.setChildrenCollapsible(False)
            new_content.addWidget(self.tabs)
            new_content.addWidget(self.terminal)
            new_content.addWidget(self.doc_panel)
            new_content.setStretchFactor(0, 1)
            new_content.setStretchFactor(1, 0)
            new_content.setStretchFactor(2, 0)
        else:
            top_splitter = QSplitter(Qt.Orientation.Horizontal)
            top_splitter.setChildrenCollapsible(False)
            top_splitter.addWidget(self.tabs)
            top_splitter.addWidget(self.doc_panel)
            top_splitter.setStretchFactor(0, 1)
            top_splitter.setStretchFactor(1, 0)

            new_content = QSplitter(Qt.Orientation.Vertical)
            new_content.setChildrenCollapsible(False)
            new_content.addWidget(top_splitter)
            new_content.addWidget(self.terminal)
            new_content.setStretchFactor(0, 1)
            new_content.setStretchFactor(1, 0)

        self.content_splitter = new_content
        if old_content_splitter is not None:
            replaced = self.splitter.replaceWidget(1, new_content)
            if replaced is not None:
                replaced.deleteLater()
        else:
            self.splitter.addWidget(new_content)
    ...
    def _setup_menus(self) -> None:
        ...
        language_guide_action = help_menu.addAction("Language &Guide")
        language_guide_action.triggered.connect(
            lambda: self._show_doc("LANGUAGE_GUIDE.md", "Language Guide")
        )

        language_spec_action = help_menu.addAction("Language &Spec")
        language_spec_action.triggered.connect(
            lambda: self._show_doc("LANGUAGE_SPEC.md", "Language Spec")
        )

        ide_guide_action = help_menu.addAction("&IDE Guide")
        ide_guide_action.triggered.connect(
            lambda: self._show_doc("IDE_GUIDE.md", "IDE Guide")
        )
    ...
    def _setup_toolbar(self) -> None:
        ...
        toolbar.addAction("Build", self._build_code)
    ...
    def _file_dialog_filter(self) -> str:
        extensions = self.language_provider.file_extensions
        if not extensions:
            return "All Files (*)"
        patterns = " ".join(f"*{ext}" for ext in extensions)
        return f"{self.language_provider.name} Files ({patterns});;All Files (*)"

    def _open_file_dialog(self) -> None:
        path, _ = QFileDialog.getOpenFileName(self, "Open File", "", self._file_dialog_filter())
        if path:
    ...
    def _save_current_file_as(self) -> None:
        ...
        if editor is None:
            return
        path, _ = QFileDialog.getSaveFileName(self, "Save File", "", self._file_dialog_filter())
        if not path:
            return
        try:
    ...
    def _show_doc(self, filename: str, title: str) -> None:
        self.doc_panel.show_doc(filename, title)
    ...
    def _update_title(self) -> None:
        ...
        self.setWindowTitle(f"Leopard IDE - {path_text}{dirty}")
    ...
    def _set_console_position(self, position: str) -> None:
        if position == self._console_position:
            return
        self._console_position = position
        self._rebuild_content_layout()
    ...
    def _build_code(self) -> None:
        ...
        if editor is None:
            return
        default_dir = str(editor.file_path.parent) if editor.file_path else str(Path.cwd())
        output_dir = QFileDialog.getExistingDirectory(self, "Build Output Directory", default_dir)
        if not output_dir:
            return
        name = editor.file_path.stem if editor.file_path else "leopard_program"
        self.terminal.write(f"\n> Build ({self.language_provider.name})")
        self.language_provider.compile(editor.toPlainText(), name, Path(output_dir), self.terminal)
        self.status.showMessage(f"Built with {self.language_provider.name} provider")
